...
# Thêm lệnh @reboot vào User Data để chạy khi khởi động lại
echo -e "\necho '@reboot root /bin/bash /var/lib/cloud/instance/scripts/part-001' | sudo tee -a /etc/crontab > /dev/null" >> "$user_data_file"
...
for region in "${regions[@]}"; do
    ...
    for instance_id in $instance_ids; do
        ...
        echo "Instance $instance_id đã chạy lại thành công và sẽ tự động thực thi User Data!"
    done
done
